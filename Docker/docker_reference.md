...
-   Docker images can be created using 2 ways: (1) start with an
...
-   How to change file permission mode generated by docker container?
    ...
    Alternatively, one can add the option “-u
    ![(id -u):](https://latex.codecogs.com/png.image?%5Cdpi%7B110%7D&space;%5Cbg_white&space;%28id%20-u%29%3A "(id -u):")(id
    -g)” to the command “docker run”, other than modify the image, but
    note that this approach may limit access to some files or folders
    owned by root user in the container.
...
-   How to clone a private repo in Dockerfile?
    ...
    When one need to download some code from a private repo when
