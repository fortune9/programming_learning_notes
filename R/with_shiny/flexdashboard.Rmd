...
One can also use the function [shiny::column()] to add columns
to any chart. 
...
- social links: one can provide option 'social' to the output head section to include
	...
	Each of these will generate a shareable link to the flexdashboard page. One can also
	just use the keyword 'menu' to include all of these social media.
...
Compared to shiny, one only need provide output content with functions like
renderXXX() and the UI elements are implicitly and automatically generated
by rmarkdown (though one can explicitly to specify UIs if he likes). For
example, one can call `renderPlot()` without calling `plotOutput()` to generate
the UI element beforehand. In this case, when one want to pass the parameters
to `plotOutput()`, he can use the argument `outputArgs` in `renderPlot()`.
However, when using `observeEvent()` or `observe()`, one need call `xxxxOutput()`
function to place the output, otherwise no output to show up.
...
**Important**: the global chunk will be the first one to execute when compiling
the document, so it can't rely on the results from other chunks, otherwise the
compiling will fail.
...
Although flexdashboard will wrap these outputs into containers very well
without UI specification, but one can provide both UI and server-side
function at the same time, which is useful when one wants to update the
output based one events, like an example below:

```
# UI
actionButton("update","Click to update")
textOutput("outText") # needed, otherwise no output
# server side
observeEvent(input$update, {
	output$outText<-renderText("Updated text")
	}
)
```

For this example, one may just use `renderText()` directly to
...
One can also use `[shiny::shinyApp()]` to embed a shiny app in
dashboard. In contrast shiny modules, this inline shiny app
is included in a page using $\lt iframe \gt$.

#### External shinyApp

Alternatively, one can also embed external shinyApp into
dashboard using the function `[shiny::shinyAppDir()]`. This provides
a good way to reuse existing applications. One example can
be found at [here](https://bookdown.org/yihui/rmarkdown/shiny-embedded.html).
